...
save_model_path2 = "ev_charger_types_classifier_2.pth"  # Path to the saved model
...
class EvChargerTypesClassifier2(nn.Module):
    def __init__(self, num_classes=2):
        super(EvChargerTypesClassifier2, self).__init__()
        self.conv_layers = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.25),
...
model2 = EvChargerTypesClassifier2(len(classes)).to(device)
...
def test_model2(image_path):
    # Load the model weights
    model2.load_state_dict(torch.load(save_model_path2))
    ...
    # Perform prediction
    with torch.no_grad():
        outputs = model2(image.to(device))
